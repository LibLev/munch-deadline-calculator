...
export class DeadlineCalculatorService {
  ...
  calculateDeadline(dto: ReportDto) {
    ...
    const result = this.getTmpResult(startDate, dto.duration);

    // Check if the result is after working time
    const isAfterWorkingTime = result.getHours() >= this.END_OF_WORK && result.getMinutes() > 0;

    return isAfterWorkingTime ? this.deadlineOnTheNextDay(result) : result.toLocaleString();
  }
  ...
  private checkStartDate(start: Date) {
    if (
      start.getDay() === 6 ||
      start.getDay() === 7 ||
